...
d3.json(url).then(
    function(data){
        ...
        for(let i = 0; i < data.length; i++){
            ...
            if(location){
                // add the marker to the map - index 1 is longitude & index 0 is latitude
                L.marker([location.coordinates[1], location.coordinates[0]]).bindPopup(data[i].descriptor).addTo(myMap);
            }
        }
    }
);